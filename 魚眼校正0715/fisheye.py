...
# 魚眼校正函式
def fisheye_to_perspective(fisheye_image, WK, HK, f, cx, cy):
    H, W = fisheye_image.shape[:2]
    perspective_image = np.zeros((HK, WK, 3), dtype=fisheye_image.dtype)
    ...
    for i in range(WK):
        for j in range(HK):
            ...
            if norm == 0:
                ...
                scale = 2 * f * np.sqrt((1 - f / np.sqrt(x**2 + y**2 + f**2)) / 2) / norm
                tx = W / 2 + scale * x
                ty = H / 2 + scale * y
            ...
            if 0 <= u < W and 0 <= v < H:
                perspective_image[j, i] = fisheye_image[v, u]
    
    return perspective_image

# 假設有一個魚眼圖像 image 和相關參數
H, W = image.shape[:2]
K = 2
WK = int(W * K)
HK = int(H * K)
...
# 呼叫函式進行魚眼到透視的轉換
perspective_image = fisheye_to_perspective(image, WK, HK, f, cx, cy)
